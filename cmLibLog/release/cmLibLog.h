...
int liblogPrintBool(HLOG hLog, bool b);	// Bool値の出力
int liblogPrintInt(HLOG hLog, int num); // 符号付き32bit整数出力
int liblogPrintUint(HLOG hLog, unsigned int num); // 符号無32bit整数出力
int liblogPrintU64(HLOG hLog, u64 num);	// 符号無64bit整数出力
int liblogPrintHex(HLOG hLog, u32 hex);	// 32bit16進数出力
int liblogPrintHex64(HLOG hLog, u64 hex); // 64bit16進数出力
